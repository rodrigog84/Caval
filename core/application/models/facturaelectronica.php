...
//			print_r($query->result());
	...
	public function get_empresa(){
		$this->db->select('rut, dv, razon_social, giro, cod_actividad, dir_origen, comuna_origen, fec_resolucion, nro_resolucion, logo, fono, mail, fec_inicio_boleta ')
		  ->from('empresa')
		  ->limit(1);
		$query = $this->db->get();
		return $query->row();
...
public function consumo_folios_no_enviada(){
	...
	public function datos_dte($idfactura){
		...
		$this->db->select('f.id, f.folio, f.path_dte, f.archivo_dte, f.archivo_dte_cliente, f.dte, f.dte_cliente, f.pdf, f.pdf_cedible, f.trackid, c.tipo_caf, tc.nombre as tipo_doc, cae.nombre as giro, if(fc.cond_venta<>"",fc.cond_venta,cp.nombre) as cond_pago, if(fc.vendedor<>"",fc.vendedor,v.nombre) as vendedor, fc.oreferencia, fc.neto, fc.iva, fc.totalfactura ',false)
		  ->from('folios_caf f')
		  ->join('caf c','f.idcaf = c.id')
		  ->join('tipo_caf tc','c.tipo_caf = tc.id')
	...
	public function datos_dte_by_trackid($trackid){
		$this->db->select('f.id, f.folio, f.path_dte, f.archivo_dte, f.dte, f.pdf, f.pdf_cedible, f.trackid, c.tipo_caf, tc.nombre as tipo_doc, cae.nombre as giro, if(fc.cond_venta<>"",fc.cond_venta,cp.nombre) as cond_pago, if(fc.vendedor<>0,fc.vendedor,v.nombre) as vendedor, fc.oreferencia, fc.neto, fc.iva, fc.totalfactura',false)
		  ->from('folios_caf f')
		  ->join('caf c','f.idcaf = c.id')
		  ->join('tipo_caf tc','c.tipo_caf = tc.id')
		  ->join('factura_clientes fc','f.idfactura = fc.id','left')
		...
		//	var_dump($datos_folio); exit;
			if(count($datos_folio) > 0){
				...
				if($docto->tipocaf == 61 || $docto->tipocaf == 56){
					...
					if($docto->tipocaf == 52 && $data_csv[0]->referencia != '' && $data_csv[0]->referencia != '0'){
						$numfactura = isset($this->get_content_caf_folio($data_csv[0]->referencia,33)->idfactura) ? $this->get_content_caf_folio($data_csv[0]->referencia,33)->idfactura : $data_csv[0]->referencia;  //referencia siempre es una factura electronica
					}else{
						$numfactura = 0;	
					}
				...
				foreach ($data_csv as $regcsv) {
					...
					if($regcsv->preciounit != 0){
						$lista_detalle[$i]['PrcItem'] = $docto->tipocaf == 39 ? floor($regcsv->totaldetalle*1.19/$regcsv->cantidad) : $regcsv->preciounit;
						$lista_detalle[$i]['MontoItem'] = $docto->tipocaf == 39 ? floor($regcsv->totaldetalle*1.19) : $regcsv->totaldetalle;
					}
				...
				if($tipodocumento == 101 || $tipodocumento == 102 || $tipodocumento == 103 || $tipodocumento == 104 || $tipodocumento == 120 || $tipodocumento == 105){  // SI ES FACTURA ELECTRONICA O NOTA DE CRÉDITO O FACTURA EXENTA ELECTRONICA O NOTA DE DEBITO
							...
							if($tipo_caf == 61){
								...
								$guia_servicios = $data_csv[0]->referencia != '' && $data_csv[0]->referencia != '0' ? true : false;
								...
								if($guia_servicios){
									...
									$factura = [
										...
										'Referencia' => [
									                    ...
									                    'TpoDocRef' => 801,
									                    'FolioRef' => $data_csv[0]->referencia,
									                    'FchRef' =>  $data_csv[0]->fechafactura
									                	] 	
									];
